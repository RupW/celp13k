...
such commitments.
*******************************************************************/
/* postfilt.c -- Post-filter after formant synthesis */
	   	...
	   	zero_wght_factor=1;
	    	...
	    	for (j=0; j<LPCORDER; j++) {
	      	...
	      	d_mem->post_filt_z.zero_coeff[j]=
					-lpc[j]*zero_wght_factor;
	      	pole_wght_factor*=PF_POLE_WGHT_FACTOR;
	      	d_mem->post_filt_p.pole_coeff[j]=
					lpc[j]*pole_wght_factor;
	    	}
	    	do_zero_filter(out_buffer, pf_out, length, 
			   &(d_mem->post_filt_z), UPDATE);
	    	do_pole_filter(pf_out, pf_out, length, 
			   &(d_mem->post_filt_p), UPDATE);
	    	do_pole_filter(pf_out, pf_out, length, &(d_mem->bright_filt),
			   UPDATE);
	    	agc(out_buffer, pf_out, out_buffer, length, 
				&(d_mem->agc_factor));
